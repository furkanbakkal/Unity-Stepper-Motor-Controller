...
def usb_connection(event=None):
        ...
        try:
            ser = serial.Serial(port=comport,baudrate = baud, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, bytesize=serial.EIGHTBITS, timeout=0.025)
            print("Connected!")
...
def go_right(event):
    ...
    print("go right")
    jobid=root.after(3000, go_right,event)
...
def go_left(event):
    ...
    jobid=root.after(3000, go_left,event)
...
#root["bg"] = "lightgrey"
bgimg= tk.PhotoImage(file = "pcb4.png")
...
def main_page():
    ...
    arrow_right=tk.Button(root, text="Button-1",height= 100, width=100,image=arrow_right_photo,borderwidth=2,bg="lightgrey",activebackground='grey')
    arrow_right.bind('<ButtonPress-1>',go_right)
    arrow_right.bind('<ButtonRelease-1>',stop_right)
    arrow_right.pack()
    ...
    arrow_left=tk.Button(root, text="Button-1",height= 100, width=100,image=arrow_left_photo,borderwidth=2,bg="lightgrey",activebackground='grey')
    arrow_left.pack()
    arrow_left.bind('<ButtonPress-1>',go_left)
    arrow_left.bind('<ButtonRelease-1>',stop_left)
...
def settings_screen():
    ...
    baudrate_choices=["9600","38400","115200"]
